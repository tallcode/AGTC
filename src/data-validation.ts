...
export function validateFFTable(data: ParsedData): ValidationResult {
  ...
  if (data.metadata.stepSize !== 1 && data.metadata.stepSize !== 0) { // 0 if only 1 point?
    ...
    errors.push(`FFtab Step Error: Step size is ${data.metadata.stepSize} degrees. Required: 1 degree.`)
  }

  // 3. Range Check
  // "FF_not_full range"
  if (!data.metadata.isFull360) {
  ...
  if (data.metadata.thetaCount !== 181) {
    errors.push(`FFtab Range Error: Elevation range is incomplete. Found ${data.metadata.thetaCount} slices, expected 181 for full sphere (0-180 deg).`)
  }
